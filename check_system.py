...
def main():
    ...
    usage_table.add_row("RAM", f"{usage['ram_used_percent']:.1f}% ({usage['ram_used_gb']:.1f}GB / {usage['ram_available_gb']:.1f}GB available)")
    usage_table.add_row("CPU", f"{usage['cpu_percent']:.1f}%")
    usage_table.add_row("Disk", f"{usage['disk_used_percent']:.1f}% ({usage['disk_free_gb']:.1f}GB free)")
