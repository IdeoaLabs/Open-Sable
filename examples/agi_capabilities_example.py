...
async def demonstrate_meta_learning(agent: AGIAgent):
    ...
    for i in range(3):
        agent.meta_learning.record_task_performance(
            ...
            metrics={
                PerformanceMetric.QUALITY: 0.4 + i * 0.2
            }
        )
...
async def demonstrate_tool_synthesis(agent: AGIAgent):
    ...
    url_tool_id = await agent.create_tool_for_task(
        ...
        expected_inputs=[
            {'name': 'url', 'type': 'str', 'description': 'URL to validate'}
        ],
        expected_outputs=[
...
async def demonstrate_world_model(agent: AGIAgent):
    ...
    agent.world_model.add_observation(
        ...
        entities=[
            ...
            {'type': 'object', 'name': 'ML Project', 'properties': {'status': 'active', 'progress': 0.6}}
        ],
        relations=[
    ...
    agent.world_model.add_observation(
        ...
        entities=[
            {'type': 'event', 'name': 'Deadline', 'properties': {'days_remaining': 14}}
        ],
        relations=[
    ...
    prediction = await agent.predict_and_plan(
        scenario="ML project completion",
        time_horizon=60
    )
...
async def demonstrate_metacognition(agent: AGIAgent):
    ...
    agent.metacognition.record_thought_step(
        trace_id, "problem_analysis",
        "Need to minimize cost while maximizing efficiency",
        raw_confidence=0.8
    )
    
    agent.metacognition.record_thought_step(
        trace_id, "approach_selection",
        "Will use linear programming approach",
        raw_confidence=0.7
    )
    
    agent.metacognition.record_thought_step(
        trace_id, "implementation",
    ...
    await agent.metacognition.complete_task(
        trace_id2,
        final_answer="Unable to determine trend",
        raw_confidence=0.3
    )
...
async def demonstrate_full_agi_cycle(agent: AGIAgent):
    ...
    if sentiment_tool:
        print(f"      ✓ Created sentiment analysis tool")
    ...
    print(f"      ✓ Working memory consolidated into long-term storage")
