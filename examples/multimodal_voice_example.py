...
async def demo_multimodal_reasoning():
    ...
    result = await agi.process_multimodal_input(
        multimodal_input,
        task_description="Provide detailed scene analysis"
    )
...
async def demo_multi_device_voice_sync():
    ...
    conversation = {
        'messages': [
            {'role': 'user', 'content': 'What is the weather like?', 'timestamp': datetime.utcnow().isoformat()},
            {'role': 'assistant', 'content': 'I can check the weather for you.', 'timestamp': datetime.utcnow().isoformat()}
        ]
    }
    
    await mobile_sync.sync_item(
        scope=SyncScope.CONVERSATIONS,
        item_id="conv_voice_001",
        data=conversation,
        version=1
    )
...
async def demo_complete_voice_agi_workflow():
    ...
    agi_agent.memory.store_experience(
        event=f"User requested image enhancement via voice command",
        context={
    ...
    print(f"  • Active goals: {len(status['subsystems']['goals'].get('by_status', {}).get('ACTIVE', []))}")
