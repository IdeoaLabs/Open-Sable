...
async def main():
    ...
    auth_span = tracer.start_span(
        "authenticate_user", SpanKind.INTERNAL, parent=parent, attributes={"user.id": "user_123"}
    )
    ...
    # Child span 3
    render_span = tracer.start_span("render_response", SpanKind.INTERNAL, parent=parent)
    ...
    service_b_span = tracer.start_span("service_b_process", SpanKind.SERVER, context=context)
    ...
    await logger.log(
        ...
        context={"version": "1.0.0", "environment": "production", "region": "us-west-2"},
    )
    ...
    await logger.log(
        ...
        context={"memory_used_mb": 850, "memory_limit_mb": 1024, "usage_percent": 83},
    )
    ...
    recent = await logger.query(
        start_time=datetime.now() - timedelta(minutes=5), end_time=datetime.now()
    )
    print(f"Recent logs (last 5 min): {len(recent)}")

    # Query by context filter
    user_logs = await logger.query(filters={"user_id": "user_456"})
    ...
    jaeger_tracer = TracingProvider(
        service_name="opensable", exporter="jaeger", endpoint="http://localhost:14268/api/traces"
    )

    # Zipkin exporter
    zipkin_tracer = TracingProvider(
        service_name="opensable", exporter="zipkin", endpoint="http://localhost:9411/api/v2/spans"
    )
    ...
    elk_logger = LogAggregator(
        service_name="opensable", backend="elasticsearch", endpoint="http://localhost:9200"
    )

    # Loki
    loki_logger = LogAggregator(
        service_name="opensable", backend="loki", endpoint="http://localhost:3100"
    )
