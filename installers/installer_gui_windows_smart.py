...
class SmartWindowsInstallerEngine:
    ...
    def _create_windows_shortcuts(self):
        ...
        helper_batches = {
            "Open-Sable-Gateway.bat": [
                "@echo off",
                "chcp 65001 >nul",
                "title Open-Sable Gateway",
                "cd /d %~dp0",
                "echo Starting Open-Sable Gateway on http://127.0.0.1:18789",
                ".venv\\Scripts\\python.exe -m opensable gateway --host 127.0.0.1 --port 18789",
                "echo.",
                "echo Open-Sable Gateway exited with code %ERRORLEVEL%",
                "pause",
            ],
            "Open-Sable-Dev-Studio.bat": [
                "@echo off",
                "chcp 65001 >nul",
                "title Open-Sable Dev Studio",
                "cd /d %~dp0sable_dev",
                "echo Starting Sable Dev Studio on http://127.0.0.1:5700",
                "set NEXT_PUBLIC_APP_URL=http://127.0.0.1:5700",
                "set PORT=5700",
                "where pnpm >nul 2>nul",
                "if \"%ERRORLEVEL%\"==\"0\" (pnpm run start -- -H 127.0.0.1 -p 5700) else (corepack pnpm run start -- -H 127.0.0.1 -p 5700)",
                "echo.",
                "echo Sable Dev Studio exited with code %ERRORLEVEL%",
                "pause",
            ],
            "Open-Sable-Dashboard.bat": [
                "@echo off",
                "chcp 65001 >nul",
                "title Open-Sable Dashboard",
                "cd /d %~dp0dashboard",
                "echo Starting Open-Sable Dashboard on http://127.0.0.1:4173",
                "if exist node_modules\\.bin\\vite.cmd (node_modules\\.bin\\vite.cmd preview --host 127.0.0.1 --port 4173) else (npm run preview -- --host 127.0.0.1 --port 4173)",
                "echo.",
                "echo Open-Sable Dashboard exited with code %ERRORLEVEL%",
                "pause",
            ],
            "Open-Sable-Aggr.bat": [
                "@echo off",
                "chcp 65001 >nul",
                "title Open-Sable Aggr Charts",
                "cd /d %~dp0aggr",
                "echo Starting Open-Sable Aggr Charts on http://127.0.0.1:5174",
                "if exist node_modules\\.bin\\vite.cmd (node_modules\\.bin\\vite.cmd preview --host 127.0.0.1 --port 5174) else (npm run serve -- --host 127.0.0.1 --port 5174)",
                "echo.",
                "echo Open-Sable Aggr Charts exited with code %ERRORLEVEL%",
                "pause",
            ],
            "Open-Sable-Desktop.bat": [
                "@echo off",
                "chcp 65001 >nul",
                "title Open-Sable Desktop App",
                "cd /d %~dp0desktop",
                "echo Starting Open-Sable Desktop App...",
                "if exist node_modules\\.bin\\electron.cmd (node_modules\\.bin\\electron.cmd .) else (npm run start)",
                "echo.",
                "echo Open-Sable Desktop App exited with code %ERRORLEVEL%",
                "pause",
            ],
        }
        for helper_name, helper_lines in helper_batches.items():
            helper_path = os.path.join(self.install_dir, helper_name)
            with open(helper_path, "w", encoding="utf-8", errors="replace") as f:
                f.write("\r\n".join(helper_lines) + "\r\n")
        ...
        launcher = [
            ...
            ")",
            "echo.",
            "echo Starting all Open-Sable services automatically...",
            "echo Starting Gateway...",
            "start \"Open-Sable Gateway\" /min \"%~dp0Open-Sable-Gateway.bat\"",
            "echo Starting Sable Dev Studio...",
            "start \"Open-Sable Dev Studio\" /min \"%~dp0Open-Sable-Dev-Studio.bat\"",
            "echo Launching Open-Sable Desktop App immediately...",
            "start \"Open-Sable Desktop App\" \"%~dp0Open-Sable-Desktop.bat\"",
            "echo Starting Dashboard...",
            "start \"Open-Sable Dashboard\" /min \"%~dp0Open-Sable-Dashboard.bat\"",
            "echo Starting Aggr Charts...",
            "start \"Open-Sable Aggr Charts\" /min \"%~dp0Open-Sable-Aggr.bat\"",
            "echo.",
            "echo All Open-Sable components were started automatically.",
            "echo You can close this launcher window after the Desktop App opens.",
            "echo ==== Auto-launch completed %DATE% %TIME% ====>>\"%LAUNCH_LOG%\"",
            "exit /b 0",
        ]
