...
class Analytics:
    ...
    def track_message_received(self, channel: str, user_id: str, message: str):
        ...
        self.track_event('message_received', {
            'channel': channel,
            'user_id': user_id,
            'message_length': len(message)
        })
    
    def track_message_sent(self, channel: str, user_id: str, message: str, 
    ...
    def track_session_created(self, channel: str, user_id: str):
        ...
        self.track_event('session_created', {
            'channel': channel,
            'user_id': user_id
        })
    ...
    def track_response_time(self, duration: float):
        ...
        self.track_event('response_time', {
            'duration': duration
        })
    ...
    def get_events(self, 
                   ...
                   limit: int = 100) -> List[MetricEvent]:
        ...
        try:
            ...
            for user_id, user_data in data.get('user_activity', {}).items():
                self.user_activity[user_id] = {
                    ...
                    'first_seen': datetime.fromisoformat(user_data['first_seen']) if user_data.get('first_seen') else None,
            ...
            logger.info(f"Loaded analytics data: {len(self.events)} events, {len(self.user_activity)} users")
