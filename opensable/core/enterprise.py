...
try:
    ...
    logging.getLogger(__name__).warning(
        "PyJWT not installed. SSO/token features disabled. "
...
class MultiTenancy:
    ...
    def __init__(self, storage_dir: Optional[str] = None):
        ...
        self.storage_dir = Path(storage_dir) if storage_dir else Path.home() / ".opensable" / "tenants"
...
class AuditLogger:
    ...
    def __init__(self, storage_dir: Optional[str] = None):
        ...
        self.storage_dir = Path(storage_dir) if storage_dir else Path.home() / ".opensable" / "audit"
        self.storage_dir.mkdir(parents=True, exist_ok=True)
...
async def main():
    ...
    for log in logs:
        print(f"    - {log.timestamp.strftime('%Y-%m-%d %H:%M:%S')} | {log.action.value} | {log.resource_type}")
