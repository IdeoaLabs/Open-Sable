    ...
    async def auto_switch_model(self, task_type: str) -> bool:
        ...
        if best_model and best_model != self.current_model:
            ...
            if "70b" in best_model.lower() or "405b" in best_model.lower():
                logger.warning(f"Blocked download of large model {best_model}. Using {self.current_model} instead.")
                return False
        ...
        if config.auto_select_model:
            ...
            try:
                ...
                if any(recommended in a or a in recommended for a in available):
                    ...
                    logger.info(f"Auto-selected model: {model_to_use} (tier: {auto_config['device_tier']})")
                else:
                    logger.warning(f"Auto-selected model '{recommended}' not available locally. Using default: {model_to_use}")
            except Exception:
