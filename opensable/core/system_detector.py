...
class ModelSelector:
    ...
    def can_run_model(specs: DeviceSpecs, model_name: str) -> bool:
        ...
        if specs.ram_gb < reqs["min_ram"]:
            logger.warning(f"Insufficient RAM for {model_name}: {specs.ram_gb}GB < {reqs['min_ram']}GB")
            return False
        
        # Check GPU VRAM if using GPU
        if specs.gpu_available and specs.gpu_memory_gb < reqs.get("vram", 0):
            logger.warning(f"Insufficient VRAM for {model_name}: {specs.gpu_memory_gb}GB < {reqs['vram']}GB")
