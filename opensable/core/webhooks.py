...
class WebhookManager:
    ...
    async def emit(self, event: str, payload: dict):
        ...
        matching_webhooks = [
            ...
            if event in wh.events and wh.enabled
        ]
    ...
    async def _deliver(self, webhook: Webhook, event: str, payload: dict):
            ...
            try:
                async with self.session.post(
                    ...
                    if 200 <= response.status < 300:
                        ...
                        logger.warning(f"Webhook delivery failed: {webhook.webhook_id} - {delivery.error}")
        ...
        logger.error(f"Webhook delivery failed after {delivery.attempts} attempts: {webhook.webhook_id}")
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        # Emit event
        await manager.emit("message.received", {
            "user_id": "123",
            "message": "Hello!",
            "channel": "telegram"
        })
