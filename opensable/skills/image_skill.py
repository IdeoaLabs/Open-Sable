    ...
    async def _generate_stable_diffusion(
        ...
        try:
            ...
            for i, image in enumerate(images):
                ...
                # Save to cache
                cache_key = hashlib.sha256(f"{prompt}_{i}_{datetime.now().isoformat()}".encode()).hexdigest()[:16]
    ...
    async def _generate_dalle(
        ...
        try:
            ...
            for _ in range(num_images):
                ...
                # Save to cache
                cache_key = hashlib.sha256(f"{prompt}_{datetime.now().isoformat()}".encode()).hexdigest()[:16]
                cache_path = self.cache_dir / f"{cache_key}.png"
...
class OCREngine:
    ...
    async def _extract_paddle(self, image_path: str) -> OCRResult:
        ...
        if not PADDLEOCR_AVAILABLE:
            return OCRResult(
                success=False,
                error="paddleocr not installed: pip install paddleocr"
            )
